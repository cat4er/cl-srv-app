...
exmpl_files = ['info_1.txt', 'info_2.txt', 'info_3.txt']
exmpl_columns = {'Изготовитель системы': None, 'Название ОС': None, 'Код продукта': None, 'Тип системы': None}


def get_data_csv(files, columns):  # второе рещение 1 задания без дополнительных функций и списков
    new_file = open('dataset.csv', 'w')
    writer = csv.DictWriter(new_file, fieldnames=columns.keys())
    writer.writeheader()
    for file in files:
        with open(f'{file}', encoding='cp1251') as f_n:
            for row in f_n:
                for key in columns.keys():
                    if re.findall(rf'{key}', row):
                        egg = {f'{key}': re.sub(r'^\s+|\n|\r|\s+$', '', re.findall(rf'{key}:(\s.*)', row)[0])}
                        columns.update(egg)
        writer.writerow(columns)


get_data_csv(exmpl_files, exmpl_columns)