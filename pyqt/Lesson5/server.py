...
class Main:
    ...
    def tokenization(self, data):
        ...
        if 'action' in data.keys():
            ...
            if action == 'registration':
                ...
                ecb_token = AES.new(key, AES.MODE_CFB, self.salt).encrypt(data.get('user_data'))  # соль не меняется
    ...
    def registration(self, client, data):
        ...
        reg_resp = {guid: {
                    'response': 200,
                    'time': f'{datetime.now()}',
                    'alert': f'{user_name} подключен'
                    }}
        client.send(json.dumps(reg_resp).encode())
